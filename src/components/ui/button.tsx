...
const buttonVariants = cva(
  "ui:group/button ui:inline-flex ui:shrink-0 ui:items-center ui:justify-center ui:rounded-lg ui:border ui:border-transparent ui:bg-clip-padding ui:text-sm ui:font-medium ui:whitespace-nowrap ui:transition-all ui:outline-none ui:select-none ui:focus-visible:border-ring ui:focus-visible:ring-3 ui:focus-visible:ring-ring/50 ui:active:not-aria-[haspopup]:translate-y-px ui:disabled:pointer-events-none ui:disabled:opacity-50 ui:aria-invalid:border-destructive ui:aria-invalid:ring-3 ui:aria-invalid:ring-destructive/20 ui:dark:aria-invalid:border-destructive/50 ui:dark:aria-invalid:ring-destructive/40 ui:[&_svg]:pointer-events-none ui:[&_svg]:shrink-0 ui:[&_svg:not([class*='size-'])]:size-4",
  {
    variants: {
      variant: {
        default: "ui:bg-primary ui:text-primary-foreground ui:hover:bg-primary/80",
        outline:
        ...
        secondary:
          "ui:bg-secondary ui:text-secondary-foreground ui:hover:bg-[color-mix(in_oklch,var(--ui-secondary),var(--ui-foreground)_5%)] ui:aria-expanded:bg-secondary ui:aria-expanded:text-secondary-foreground",
        ghost:
      ...
      size: {
        ...
        xs: "ui:h-6 ui:gap-1 ui:rounded-[min(var(--ui-radius-md),10px)] ui:px-2 ui:text-xs ui:in-data-[slot=button-group]:rounded-lg ui:has-data-[icon=inline-end]:pr-1.5 ui:has-data-[icon=inline-start]:pl-1.5 ui:[&_svg:not([class*='size-'])]:size-3",
        sm: "ui:h-7 ui:gap-1 ui:rounded-[min(var(--ui-radius-md),12px)] ui:px-2.5 ui:text-[0.8rem] ui:in-data-[slot=button-group]:rounded-lg ui:has-data-[icon=inline-end]:pr-1.5 ui:has-data-[icon=inline-start]:pl-1.5 ui:[&_svg:not([class*='size-'])]:size-3.5",
        lg: "ui:h-9 ui:gap-1.5 ui:px-2.5 ui:has-data-[icon=inline-end]:pr-2 ui:has-data-[icon=inline-start]:pl-2",
        icon: "ui:size-8",
        "icon-xs":
          "ui:size-6 ui:rounded-[min(var(--ui-radius-md),10px)] ui:in-data-[slot=button-group]:rounded-lg ui:[&_svg:not([class*='size-'])]:size-3",
        "icon-sm":
          "ui:size-7 ui:rounded-[min(var(--ui-radius-md),12px)] ui:in-data-[slot=button-group]:rounded-lg",
        "icon-lg": "ui:size-9",
      },
    },
    defaultVariants: {
